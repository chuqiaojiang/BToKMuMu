...
// --- begin input from python file ---
	...
	double BuMass_;
	// labels
	edm::EDGetTokenT<vector<reco::GenParticle>> 		GenParticlesLabel_;
	edm::EDGetTokenT<edm::TriggerResults> 			TriggerResultsLabel_;
	edm::EDGetTokenT<reco::BeamSpot> 			BeamSpotLabel_;
	edm::EDGetTokenT<reco::VertexCollection> 		VertexLabel_;
	edm::EDGetTokenT< vector<pat::Muon> > 			MuonLabel_;
	edm::EDGetTokenT< vector<pat::PackedCandidate> >	TrackLabel_;
...
// --- end input from python file ---
	...
	vector<int> *mumtrkqual, *muptrkqual;    /* added quality vars 2015-10-09 */
...
BToKMuMu::BToKMuMu(const edm::ParameterSet& iConfig):
  ...
  // labels
  GenParticlesLabel_(consumes<vector<reco::GenParticle> >(iConfig.getParameter<edm::InputTag>("GenParticlesLabel"))),
  ...
  MuonLabel_(consumes< vector<pat::Muon> >(iConfig.getParameter<edm::InputTag>("MuonLabel"))),
  TrackLabel_(consumes< vector<pat::PackedCandidate> >(iConfig.getParameter<edm::InputTag>("TrackLabel"))),
  ...
  mumtrkqual(0), muptrkqual(0),         /* added on quality vars 2015-10-09 */
...
BToKMuMu::beginJob()
{
  ...
  tree_->Branch("mumtrkqual", &mumtrkqual);  /* added quality vars 2015-10-09 */
  tree_->Branch("muptrkqual", &muptrkqual);  /* added quality vars 2015-10-09 */
...
BToKMuMu::clearVariables(){
  ...
  mumtrkqual->clear(); muptrkqual->clear();   /* added on 2015-10-09 */
...
//？？？what is innerTrack
    ...
    const reco::TransientTrack muTrackmTT((*theB).build(muTrackm));;
    ...
    for (vector<pat::Muon>::const_iterator iMuonP = patMuonHandle->begin();
      ...
      // check mu+ DCA to beam spot
      const reco::TransientTrack muTrackpTT((*theB).build(muTrackp));
      ...
      */
      ...
      if ( !passed) continue;
        ...
        for ( vector<pat::PackedCandidate>::const_iterator iTrack
         ...
         const reco::TransientTrack theTrackTT((*theB).build(kaonTrack));
                                 ...
                                 reco::TrackRef muTrackm, reco::TrackRef muTrackp)
{
  ...
  mumtrkqual->push_back(muTrackm->quality(reco::TrackBase::highPurity)); /*added on 2015-10-09 */
  muptrkqual->push_back(muTrackp->quality(reco::TrackBase::highPurity)); /*added on 2015-10-09 */
